...
import { Plus, Search, Store, Trash2, Minus, ChevronDown } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useI18n } from '@/lib/i18n';
import { useProducts, useShoppingList, useStores, usePurchaseHistory } from '@/lib/useStore';
...
export default function ShoppingListPage() {
  ...
  const { items, addItem, removeItem, toggleCheck, updateQuantity, clearChecked, total, activeStoreId, setActiveStoreId } = useShoppingList();
  ...
  const handlePriceConfirm = (price?: number, discount?: number) => {
    ...
    if (price && activeStoreId) {
      addRecord({ productId: priceItem.productId, price, discount: discount || 0, storeId: activeStoreId, date: new Date().toISOString() });
    }
    setPriceItem(null);
  };
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  {t('checkedItems')} ({checkedItems.length})
                </span>
